...
> **审查日期**：2026-09-12（初版）| **更新日期**：2026-09-12（2.2.1，重构后同步）| **代码规模**：~22,000 行 Python（不含测试）| **测试**：388 收集 / 380 通过

本次审查覆盖 52 源文件、5 个子系统（核心算法 / 服务层 / Worker 层 / 数据模型 / GUI），共发现 **5 个 Critical**、**15 个 High**、**15 个 Medium**、**17 个 Low** 级问题。最严峻的挑战在于核心渲染函数 `render_design` 达 848 行、OCR 数字识别函数达 747 行。**2.2.1 重构已将 service 层、worker 层、model 层从 core/gui 中拆分独立**，解决了 H-10（MVC 分离）等架构问题，但超长函数和部分线程安全隐患仍未修复。
...
> **2.2.1 变更要点**：新增 `services/`（服务层）、`workers/`（Worker 层）、`models/`（数据模型层）三个顶层包，将原 `core/` 中的解析器/匹配器/PSD 加载器迁移至 `services/`，将原 `gui/` 中的所有 QThread Worker 迁移至 `workers/`，新建 `models/design_model.py` 提供 DesignModel 中间层。清理了 50+ 失效诊断脚本和超期归档测试。

### 项目目录结构

| 目录 | 职责 | 文件数 | 总行数 | 变更 |
|------|------|--------|--------|------|
| `core/` | 核心算法（渲染 / 裁剪 / 几何 / 边框 / 圆角） | 20 | 7,564 | 缩减（解析器迁出） |
| `services/` | 服务层（模板匹配 / 草图解析 / PSD 加载） | 15 | 8,072 | **新增** |
| `gui/` | UI 面板 / 画布 | 11 | 5,262 | 缩减（Worker 迁出） |
| `workers/` | 线程调度层（所有 QThread Worker） | 4 | 1,004 | **新增** |
| `models/` | 数据模型层（DesignModel 包装器） | 2 | 58 | **新增** |
| `tests/` | 单元测试 / 集成测试 | 35 | 7,141 | 扩充 |
| `packaging/` | PyInstaller 打包脚本 | 5 | 1,828 | legacy/ 归档 |

> **核心架构亮点**：2.2.1 实现了五层分离架构——`models/`（纯数据）← `services/`（业务逻辑）← `workers/`（线程调度）← `gui/`（UI 展示）← `core/`（算法内核）。Worker 层不再导入任何 gui 模块，services 层不依赖 UI，DesignModel 提供 `sync_from_design()` / `to_design()` 替代 main.py 直接操作 Panel 私有控件。配置集中在 `core/config.py`、跨平台路径解析器 `PathResolver`、结构化日志、全局异常钩子、QThread + 信号槽的异步渲染管线、LOD 分级预览、设计对象快照防竞态等基础设施保持不变。
...
| `core/image_ops.py` | 1626 | :red_circle: 需拆分 | 不变 |
| `services/parser/template_matcher.py` | 1268 | :orange: 偏大 | 路径迁移 core→services |
| `services/sketch_parser/lshape_sketch_parser.py` | 1121 | :orange: 偏大 | 路径迁移 core→services |
| `gui/cropper_panel.py` | 992 | :orange: 偏大 | 微增（+~58 行） |
| `services/sketch_parser/sketch_parser_numbers.py` | 982 | :orange: 偏大 | 路径迁移 core→services |
| `gui/property_panel_poolbox.py` | 944 | :orange: 偏大 | 不变 |
| `core/lshape_border.py` | 933 | :yellow_circle: 可接受 | 不变 |
| `workers/property_panel_workers.py` | 842 | :yellow_circle: 可接受 | 路径迁移 gui→workers |
...
> **注**：2.2.1 重构未拆分超长函数，以下行号基于迁移后的新路径。

| 文件 | 函数 | 行号 | 行数 | 严重性 | 2.2.1 变更 |
|------|------|------|------|--------|------|
| `core/image_ops.py` | `render_design` | L550-1397 | **848** | :red_circle: Critical | 路径不变 |
| `services/sketch_parser/sketch_parser_numbers.py` | `_extract_direction_label_numbers` | L274-1020 | **747** | :red_circle: Critical | core→services |
| `services/sketch_parser/lshape_sketch_parser.py` | `_detect_lshape_geometry` | L104-676 | **573** | :red_circle: Critical | core→services |
| `workers/property_panel_workers.py` | `PoolRenderWorker.run` | L319-833 | **515** | :red_circle: Critical | gui→workers |
| `services/sketch_parser/sketch_parser_multihole.py` | `_9step_multi_hole_parse` | L1487-1896 | 410 | :orange: High | core→services |
| `services/sketch_parser/sketch_parser_margins.py` | `_validate_and_fix_margins` | L191-582 | 392 | :orange: High | core→services |
| `services/sketch_parser/sketch_parser_multihole.py` | `_classify_hole_layout` | L136-492 | 357 | :orange: High | core→services |
| `services/sketch_parser/sketch_parser.py` | `_7step_parse` | L82-428 | 347 | :orange: High | **新发现** |
| `core/corner/sector_render.py` | `_redraw_border_on_corner` | L157-490 | 334 | :orange: High | 不变 |
| `gui/property_panel_layers.py` | `_collect` | L68-366 | 299 | :orange: High | **新发现** |
| `gui/property_panel.py` | `_build_ui` | L105-389 | 285 | :orange: High | 不变 |
| `services/sketch_parser/lshape_sketch_parser.py` | `_assign_labels_by_geometry` | L684-951 | 268 | :orange: High | core→services |
| `core/image_cropper_mask.py` | `_post_cleanup_gap_regions` | L385-647 | 263 | :orange: High | 不变 |
| `gui/cropper_panel.py` | `_build_ui` | L73-322 | 250 | :orange: High | 行号变化 |
| `services/sketch_parser/sketch_parser_multihole.py` | `_extract_arrow_direction_numbers` | L784-1029 | 246 | :orange: High | core→services |
| `services/sketch_parser/sketch_parser_multihole.py` | `_divide_multi_hole_zones` | L500-723 | 224 | :orange: High | core→services |
| `core/corner/detection.py` | `_detect_border_layers` | L503-725 | 223 | :orange: High | 不变 |
| `core/image_cropper_border.py` | `apply_border_only_corners` | L249-465 | 217 | :orange: High | **新发现** |
| `core/image_cropper_mask.py` | `_build_multi_layer_corner_mask` | L57-273 | 217 | :orange: High | 不变 |
| `gui/property_panel_poolbox.py` | `_on_sketch_parsed` | L731-946 | 216 | :orange: High | **新发现** |

共发现 **77 个**超过 80 行的函数，其中 **5 个超过 400 行**，**22 个超过 200 行**。超长函数数量较 V2.2（57 个）增加 20 个，主要因为 AST 分析覆盖了更完整的函数集（含新路径文件和 GUI mixin 内的函数）。这些超长函数仍是代码维护的最大障碍。
...
- **位置**：`services/sketch_parser/sketch_parser_numbers.py` L274-1020（原 `core/pool_designer/sketch_parser_numbers.py`）
...
### C-03: `_detect_lshape_geometry` 函数达 573 行

- **位置**：`services/sketch_parser/lshape_sketch_parser.py` L104-676（原 `core/pool_designer/lshape_sketch_parser.py`）
- **问题**：L 形草图几何检测函数包含轮廓分析、凹角检测（凸包 + 滑窗）、方向识别、尺寸解析、自洽评分等完整流程。函数过长导致每个检测阶段难以单独测试和修改。
- **建议**：拆为轮廓分析、OCR 数值提取、几何自洽、L 形参数计算四个独立模块，每阶段返回中间结构。
- **2.2.1 状态**：⛔ 未修复（仅路径迁移，行数微减 574→573）

### C-04: `PoolRenderWorker.run` 函数达 515 行

- **位置**：`workers/property_panel_workers.py` L319-833（原 `gui/property_panel_workers.py`）
...
- **2.2.1 状态**：⛔ 未修复（已从 gui 迁至 workers 层，但函数体未拆分）
...
- **2.2.1 状态**：✅ 已修复 — 所有公开方法（`scan_library`、`find_best_match`、`set_template_dir`、`clear_cache`、`get_library_stats`、`get_index_stats`）已用 `threading.RLock` 保护，采用委托模式避免缩进问题。内部调用链的可重入性风险已通过锁粒度控制缓解。
...
- **2.2.1 状态**：🔶 部分解决 — services 层（模板匹配、草图解析）已从 GUI 迁出；models/design_model.py 已创建 DesignModel 中间层。但 `_collect()` 仍直读 UI 控件，尚未改为 Model 驱动。
...
- **2.2.1 状态**：🔶 部分解决 — DesignModel 已提供 `sync_from_design()` 接口，但 main.py 尚未改为调用 model 而非直接操作 Panel 控件。
...
- **2.2.1 状态**：⛔ 未修复（已迁移至 workers 层，但取消回调仍缺失）
...
- **2.2.1 状态**：⛔ 未修复（已迁移至 workers 层，但语义问题仍在）
...
- **2.2.1 状态**：🔶 部分解决 — `packageV2.1.2.py` 已移入 `packaging/legacy/`，但 `package.py`/`packageV2.0.py`/`packageV2.1.py` 仍在根目录。
...
| Worker | 所在位置 | 退役模式 | 取消支持 | 评估 | 2.2.1 变更 |
|--------|----------|----------|----------|------|------|
...
| `PoolRenderWorker` | `workers/property_panel_workers.py` | 同上 | ✅ 已实现 | ⚠️ 职责过重 | gui→workers |
...
> **线程安全总结**：QThread 退役模式整体正确——所有 Worker 都通过 `requestInterruption()` + `wait()` + `deleteLater()` 安全退役。`closeEvent` 和 `app.aboutToQuit` 双重接管所有后台线程。2.2.1 中所有 Worker 已统一迁移至 `workers/` 包，不再导入 gui 模块。**剩余风险点**：**(1)** `AutoMatchWorker` 缺少 `check_cancel` 回调；**(2)** `_SketchDecodeWorker` 中断检查语义混淆；**(3)** `PoolRenderWorker.run` 515 行职责过重。**已修复**：TemplateMatcher RLock 可重入性风险已解决。
...
> **测试覆盖关键缺口**：`sketch_parser_numbers.py`（747 行 OCR 核心函数）和 `sketch_parser_cache.py` 完全没有独立单元测试，是最大的覆盖盲区。`render_design`（848 行）的端到端渲染路径也缺少直接测试覆盖。2.2.1 新增的 border 模式安全测试（446+337 行）显著提升了边框渲染的回归保护。
...
| 项 | 状态 |
|---|---|
| **C-01**: 拆分 `render_design`（848 行）为 5-7 个子函数，使用 RenderContext 承载参数 | ⛔ 未修复 |
| **C-02**: 拆分 `_extract_direction_label_numbers`（747 行）为 OCR 扫描框架 + 规则策略表 | ⛔ 未修复 |
| **C-03**: 拆分 `_detect_lshape_geometry`（573 行）为四阶段独立模块 | ⛔ 未修复 |
| **C-04**: 拆分 `PoolRenderWorker.run`（515 行）为子步骤，每步返回中间结果 | ⛔ 未修复 |
...
> 以下为 V2.2 → 2.2.1 期间（2026-09-12 commit `558b54c` → `9e7c1c0`）的架构变更总结。
...
*版本历史：V2.2（2026-09-12 初版，基于 2 层架构）→ 2.2.1（2026-09-12 重构同步，基于 5 层架构）。*

*注意：本报告基于 2026-09-12 的代码快照（commit `9e7c1c0`）。后续代码变更可能影响发现项的有效性。*
