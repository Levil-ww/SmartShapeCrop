...
3. **三种模式区分判定**(V2.3 新增 — 基于 2026-09-18 GUI 设计讨论):
   - **概念区分**(以 CutRect 列表特征纯几何判定,无需用户指定):

     | 模式 | CutRect 列表特征 | anchor | offset | 典型场景 |
     |---|---|---|---|---|
     | **① 单边 L 形** | 1 笔 | 任意 | (0,0) | 单角避让(鼠标/电源线) |
     | **② 多边 L 形** | N 笔(N≤4) | **不同** | 全 0 | 多角圆角/四角避让 |
     | **③ 阶梯 L 形**(新) | N 笔(N≤3) | **相同** | **递增非零** | 同角多级台阶(安妮森林) |

   - **判定规则**:
     - `len(l_cut_rects) == 1` → ① 单边
     - `len > 1 and 所有 anchor 相同` → ③ 阶梯
     - `len > 1 and anchor 不全同` → ② 多边

   - **各层影响**:

     | 影响层 | ① 单边 | ② 多边 | ③ 阶梯(新) |
     |---|---|---|---|
     | 数据模型 | ✓ 无影响(CutRect 列表统一表达) | ✓ 无影响 | ✓ 无影响(offset=0 子集,向后兼容) |
     | GUI 交互 | ✓ 无影响 | ✓ 无影响 | △ 新增模式选择器 + 同角位子行 |
     | 识别层 | ✓ 无影响 | △ 保留桶合并(每桶 1 凹点) | △ 解除桶合并 + G1 反拼 IoU≥0.92 |
     | 渲染层 | ✓ 无影响(挖空矩形) | ✓ 无影响(挖空矩形) | ✗ 凹角补边需改(挖空+凸回两步) |

   - **关键约束**:三种模式**互斥**,一期**不支持「阶梯 + 多边混合」**(如 tr 阶梯 + bl 单挖角,见 Revisit Triggers)

4. **GUI 阶梯模式使用流程**(V2.3 新增,V2.4 修订尺寸输入语义与 G1 闸口 — 5 步操作,对应分期四 GUI 实施):
   - **操作步骤**:

     | 步 | 操作 | 控件 | 说明 |
     |---|---|---|---|
     | ① | 选模式 | 顶部模式选择器(单选) | 默认识别自动锁定,也可手动切到「阶梯 L 形」 |
     | ② | 选角位 | tl/tr/bl/br 4 选 1 | **整阶梯共用一个角位**(不支持多角混合) |
     | ③ | 设级数 | +/− SpinBox | 默认 2 级(下限),上限 3 级 |
     | ④ | 填尺寸 | 每级「步进宽 × 落差」SpinBox(V2.4 修订) | 第 1 级 = 远端第一步,依次向角位;第 2 级行缩进显示 |
     | ⑤ | 应用预览 | 「应用并预览」按钮 | 触发 200ms debounce → `_render_async` |

   - **关键交互设计**:
     - **模式自动锁定**:上传草图后,识别层根据 `cuts_cm` 自动判定(anchor 全同 + offset 非零 → 锁定阶梯模式);用户也可手动切换(会清空参数)
     - **参数只读 + 解锁编辑**:识别回填的参数默认只读(灰色 input),需点击「解锁编辑」才能修改 — 防止误改覆盖识别结果
     - **第 2 级行缩进**:第 2 级行 `padding-left: 24px` + 「↳」前缀;第 3 级行 `padding-left: 48px`,缩进递增,视觉上表达"嵌套"关系
     - **步进值 → CutRect 换算**(V2.4 修订,替代原「offset = 前 N−1 级尺寸之和」——原规则未指明累加轴,而内缩沿深度轴累计、外扩沿宽度轴累计,单一规则无法覆盖两种形态):
       - 用户输入 = 草图标注的「步进宽 × 落差」,第 1 级 = 远端第一步,依次向角位;程序按**条带分解**统一换算,内缩/外扩形态由换算结果自然呈现,无需用户区分
       - 以「上边 + tr 角」为例:第 i 级 CutRect = `(offset_x, offset_y, w, h) = (0, Σ_{j<i} 落差_j, Σ_{j≥i} 宽_j, 落差_i)`;其余角位按镜像(上边 tl、下边 bl/br)与轴交换(左边 tl/bl、右边 tr/br)对称处理
       - 换算示例(20260918 实测草图,标注 5×7 + 15×7):→ `[(0,0,20,7), (0,7,15,7)]`,union 恰为阶梯轮廓;若按标注值直接当 CutRect 尺寸(同角 offset 0 两笔 5×7 / 15×7),两笔嵌套重叠,validate 直接拒绝,无法构成阶梯
       - 换算结果灰色只读展示;识别回填按逆换算(CutRect 条带 → 步进值)显示,与手动输入共用同一套行控件

   - **模式切换规则**:

     | 当前模式 | 切换到 | 行为 |
     |---|---|---|
     | 单边 | 阶梯 | 清空 1 行参数,展开为 2 级子行 |
     | 多边 | 阶梯 | 清空多角位行,合并为 1 角位 + 多级子行 |
     | 阶梯 | 单边 | 保留第 1 级参数,丢弃第 2/3 级 |
     | 阶梯 | 多边 | 保留第 1 级参数,丢弃第 2/3 级 |

   - **G1 闸口拆分**(V2.4 修订——原设计将反拼 IoU 放入 GUI 实时状态条,但手动参数模式没有识别轮廓可参照,IoU 无法计算,故拆为两级):
     - **识别闸口**(识别路径,有轮廓):检测凹点数 vs 消费角数、反拼 IoU ≥ 0.92、同边约束——识别时一次性判定,失败自动降级 multi_edge(第二期 B4 已实施);识别回填后在状态条展示其结果(含 IoU 值)供参考,手动编辑后不重算
     - **参数闸口**(GUI 路径,手动输入或回填后修改):仅几何校验——单一角位、级数 1–3、每级步进宽/落差 > 0、换算后 CutRect 两两不重叠且不越界(复用 `_validate_l_cut_rects`);任一失败 → 状态条变红 + 「应用并预览」按钮禁用,防止错误参数进入渲染层
