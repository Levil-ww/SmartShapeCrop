...
| 项目版本 | V2.2.1 |
| 当前基线 commit | `300c3dc` |
| 状态 | 一期/二期/三期全部完成 · 真实草图流程验证已完成 · 多角 L 形挖角功能可交付 |
| 替代 | V1.0（同日） |
...
| 当前状态 | 已实施到第 2 期相邻角冲突切片 | **一期全部完成**（收敛解除 + G1 闸口 + 多角 GUI/几何/渲染闭环 + 自动识别回填 + G2/G3 + 实时边余量反馈）；**二期全部完成**（逐角边框调度 + 对角/相邻/四角同挖回归 + 真实素材端到端验收）；**三期全部完成**（OCR 逐角归属 `_attribute_cut_ocr_per_corner` + 像素比例兜底 + 9 项测试通过）；**真实草图流程验证已完成** | 工作区基于 `300c3dc` 的增量；三期新增 `_attribute_cut_ocr_per_corner` + `_resolve_cut_pair_per_corner`，`cuts_cm` 每项含 `source` 字段区分 OCR/像素比例来源 |
...
> **2026-09-15 已实装：** G1 不变量已作为 `_apply_g1_invariant()` 函数落地，`parse_lshape_sketch` 的所有退出路径（正常完成、尺寸不完整 early return）均调用此函数。debug 字段 `g1_invariant_applied=True` 标记不变量已运行，`g1_blocked` 标记是否拦截。12 项不变量属性测试覆盖全路径（`tests/core/test_g1_invariant.py`）。
...
- [x] `compute_lshape_border_bands` 抽样点扩充（清单 #5）：已扩充为四角多点回归
- [x] 相邻角边框冲突测试：补齐 `tl+tr`、`bl+br`、`tl+bl`、`tr+br`；结果按单角结果并集验收，缺口内部保持底色
- [x] 四角同挖幂等性验收已完成；真实素材端到端验收已完成（`tests/integration/test_pool_lshape_flow.py`：7 项通过，覆盖四缺口、中心素材保留与输出尺寸）
...
> **二期已全部完成：** 逐角调度已覆盖对角、相邻角、四角同挖三种场景；底层几何与走线算法继续按单角复用，相邻重叠策略（输入顺序先到先得 + 临时画布合并 + 重叠像素只提交一次）已落地。真实素材端到端验收已通过。

### 第三期　自动识别：OCR 标注逐角归属　`3–5 天 · 原估 1–2 周`

- [x] **范围已大幅收窄**：凹角检测与尺寸配对已由第一期完成（§5），本期的真正内容是
  **把 OCR 尺寸标注归属到正确的角**
- [x] 现有 `cut_top = corner in ('tr','tl')`（`:731`）这类单布尔判定，
  泛化为「每角一组」的 A/B/C/D/E/F 角色映射
- [x] `_resolve_dimensions`（`:991`）从标量输出改为逐角列表
- [x] 同步泛化 SR-1 的归属判定权重（角越多、标注越密，误归属概率越高）
...
- [x] G1 结构一致性闸口已作为**不变量常驻**实装（§6.4）：`_apply_g1_invariant()` 函数覆盖所有退出路径（正常完成 + 尺寸不完整 early return），`debug['g1_invariant_applied']` 标记不变量已运行，12 项属性测试覆盖全路径（`tests/core/test_g1_invariant.py`）。
...
8. **已完成四角同挖与真实素材验收**：四角调用幂等性验证通过，真实边框素材覆盖 V13、Profile、旧路径三条路由，端到端验收已完成（`tests/integration/test_pool_lshape_flow.py`：7 项通过）。
9. **已完成真实草图流程验证**：真实草图完整流程（上传→识别→回填→手动修正→生成预览→导出）已跑通，覆盖低置信度告警、识别漏角 G1 拦截、手动修改后标注同步、草图切换后 overlay 清除等场景。
10. 当前可交付边界：最多 4 角的**手动几何渲染 + 预览角位核对 + 对角/相邻/四角同挖边框补全 + OCR 逐角归属**全部可用；逐边退化守卫、实时余量反馈、G1/G2/G3 已完成；真实素材端到端验收与真实草图流程验证已通过；三期 OCR 逐角归属已完成（`_attribute_cut_ocr_per_corner` + 像素比例兜底 + 9 项测试通过）。**多角 L 形挖角功能全链路可交付。**

---

*SmartShapeCrop 多角 L 形挖角可行性分析 · 报告版本 V1.1 · 2026-09-14*
*当前基线 commit 300c3dc · 项目版本 V2.2.1 · 2026-09-15 工作区已完成一期收敛解除 + G1 闸口 + 多角 GUI/几何/渲染闭环 + 二期边框补全全部完成 + 三期 OCR 逐角归属全部完成 + G1 不变量常驻实装（全退出路径覆盖 + 12 项属性测试） + 真实草图流程验证；多角 L 形挖角功能全链路可交付*
