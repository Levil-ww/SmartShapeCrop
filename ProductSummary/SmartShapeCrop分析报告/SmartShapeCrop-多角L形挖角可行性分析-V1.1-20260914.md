...
| 项目版本 | V2.2.1 |
| 当前基线 commit | `300c3dc` |
| 状态 | 第一期已完成 · 二期已完成对角双挖角边框补全首个切片 · 相邻角冲突与真实素材验收待补 |
| 替代 | V1.0（同日） |
...
| 当前状态 | 已实施到第 2 期首个切片 | **识别层收敛解除 + G1 闸口 + 数据模型/几何 mask/手动多角 GUI 最小闭环已完成**；**自动识别多角回填（`all_corners` → 4 行 GUI 建议值）已完成**；**UI 输入后的四边实时边余量提示、G2 低置信度告警、G3 状态区摘要与预览图角位画框已完成**；**素材边框补全入口已支持逐角处理 `cuts` 列表，渲染调用已传入全部挖角，对角双挖角专项回归已新增**；相邻角重叠冲突、四角同挖和真实素材端到端验收仍待完成 | 工作区基于 `300c3dc` 的增量；边框模块既有窄回归通过，双角专项回归已补入 |
...
- [ ] `compute_lshape_border_bands` 抽样点扩充、相邻角重叠冲突与四角同挖尚未完成。
...
7. **下一步优先补相邻角冲突测试**：覆盖 `tl+tr`、`bl+br`、`tl+bl`、`tr+br`，确认共享外框边段不会重复加厚、出现断口或污染缺口；必要时明确“外层优先 / 后绘制覆盖 / 合并后统一裁剪”的规则。
8. **随后做四角同挖与真实素材验收**：先验证四角调用的幂等性和边框带裁剪，再使用真实边框素材覆盖 V13、Profile、旧路径三条路由，并补充失败回退证据。
9. **最后补强真实草图验收**：覆盖低置信度、识别漏角、手动修改后标注同步、草图切换后 overlay 清除等场景。
10. 当前可交付边界：最多 4 角的**手动几何渲染 + 预览角位核对 + 对角双挖角边框补全首个切片**可用；逐边退化守卫、实时余量反馈、G1/G2/G3 已完成；相邻角/四角边框冲突和真实素材端到端连续性仍未承诺。

---

*SmartShapeCrop 多角 L 形挖角可行性分析 · 报告版本 V1.1 · 2026-09-14*
*当前基线 commit 300c3dc · 项目版本 V2.2.1 · 2026-09-15 工作区已完成手动多角几何闭环 + 自动识别回填 4 行 GUI + 实时边余量反馈 + G1/G2/G3 预览核对闭环；素材边框补全已完成对角双挖角逐角调度首个切片，相邻角冲突与真实素材验收待实施*
