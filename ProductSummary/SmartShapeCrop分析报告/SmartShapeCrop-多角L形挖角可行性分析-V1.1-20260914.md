...
| 当前状态 | 已实施到第 1 步 | **识别层收敛解除 + G1 闸口 + 数据模型/几何 mask/手动多角 GUI 最小闭环已完成**；**自动识别多角回填（`all_corners` → 4 行 GUI 建议值）已完成**；素材边框补全二期仍未完成 | 工作区基于 `300c3dc` 的增量；专项回归通过 |
...
### 第一期　打通链路：手动多角 + 解除识别收敛　`最小闭环已完成；自动识别回填待补`
...
- [ ] **仍未完成的产出**：识别结果尚未自动拆分并回填 GUI 的多行参数；当前多角参数可由用户手动填写，识别层仍按后续“自动识别回填”任务推进
...
- [ ] G2/G3 置信度与强约束告警仍待补齐；当前已完成的是多角识别、GUI 回填与逐边几何守卫的最小闭环。
...
4. **下一步实施建议：先做 UI 联动校验**：在相邻角 UI 输入尺寸后即时显示受影响边的剩余余量，但保留“生成预览”时的模型校验作为最终闸口；提示文案继续复用统一格式化函数，避免控件级校验与核心校验漂移。
5. **再补 G2/G3**：低自洽度告警；在预览或状态区展示每个识别角位、尺寸建议和逐边余量。不要把 `result.debug` 再丢弃。
6. **最后做素材边框补全二期**：先只支持对角双挖角，逐角调用现有补边逻辑并增加重叠/冲突测试；确认稳定后再开放相邻角和四角同挖。
7. 当前可交付边界：最多 4 角的**手动几何渲染**可用；逐边退化守卫和逐边错误反馈已完成；对角双挖角与四角组合已有几何回归；自动识别多角回填已完成，挖角边缘素材边框连续性仍是后续二期重点。

---

*SmartShapeCrop 多角 L 形挖角可行性分析 · 报告版本 V1.1 · 2026-09-14*
*当前基线 commit 300c3dc · 项目版本 V2.2.1 · 2026-09-15 工作区已完成手动多角几何闭环 + 自动识别回填 4 行 GUI，素材边框补全与 G2/G3 仍在后续二期实施中*
