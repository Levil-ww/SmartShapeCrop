...
| 当前状态 | 尚未实施 | **识别层收敛解除 + G1 闸口 + 数据模型/几何 mask/手动多角 GUI 最小闭环已完成**；自动识别多角回填与素材边框补全仍未完成 | 工作区基于 `300c3dc` 的增量；专项回归通过 |
...
- [ ] 自动识别结果尚未将多个角逐行回填到 GUI；G1 已完成，但 G2/G3 尚未完整落地。
...
1. **先补自动识别 → 多行 GUI 回填**：复用识别层已经产出的 `all_corners`/`notches_detected` 数据，按角位映射到 4 行；暂不做 OCR 精确归属，尺寸继续作为建议值允许人工修正。
2. **补逐边几何约束**：在 `CropDesign.validate()` 或统一几何校验入口中检查同一外边上相邻 cut 宽/高之和，阻止相邻 cut 相接、自交或退化。该项应先于开放相邻角。
3. **补 G2/G3**：低自洽度告警；在预览或状态区展示每个识别角位与尺寸建议。不要把 `result.debug` 再丢弃。
4. **再做素材边框补全二期**：先只支持对角双挖角，逐角调用现有补边逻辑并增加重叠/冲突测试；确认稳定后再开放相邻角和四角同挖。
5. 当前可交付边界：最多 4 角的**手动几何渲染**可用；对角双挖角为验收场景；自动识别多角回填和挖角边缘素材边框连续性暂不承诺。

---

*SmartShapeCrop 多角 L 形挖角可行性分析 · 报告版本 V1.1 · 2026-09-14*
*当前基线 commit 300c3dc · 项目版本 V2.2.1 · 2026-09-15 工作区已完成手动多角几何闭环，自动识别回填与素材边框补全待后续实施*
