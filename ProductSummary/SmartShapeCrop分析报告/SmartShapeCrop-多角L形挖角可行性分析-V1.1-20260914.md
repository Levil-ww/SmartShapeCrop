...
| 当前状态 | 尚未实施 | **识别层收敛解除 + G1 结构一致性闸口已完成**；数据模型、几何 mask、GUI、边框补全尚未实施 | 提交 `300c3dc`；专项测试通过 |
...
### 第一期　打通链路：手动多角 + 解除识别收敛　`识别/G1 已完成；下游闭环待确认`
...
- **仍未完成的产出**：本例双角草图进入数据模型、几何 mask、GUI 后，才能渲染正确轮廓并自动预填两个角；这是下一步候选，不视为当前已完成
...
- [ ] 数据模型列表化、几何 mask 循环化、GUI N 行、边框补全逐角走线尚未实施，也未在本轮执行。
...
1. **建议下一步先实施：数据模型 + 几何 mask + 手动多角 GUI 的最小闭环**（原第一期的下游部分）。范围限定为最多 4 个角、先支持对角双挖角；保留旧单角字段兼容。
2. 验收标准：合成草图能生成 tr 28×8 与 bl 30×12.8 两个参数，mask 轮廓正确，单角既有用例不回归。
3. **暂不实施** OCR 逐角精确归属与素材边框补全；前者放后续，后者是最大风险和第二期工作。
4. 执行前请确认：最多角数、是否只支持对角双挖角、是否需要每角独立圆角；本轮仅同步报告，等待确认。

---

*SmartShapeCrop 多角 L 形挖角可行性分析 · 报告版本 V1.1 · 2026-09-14*
*当前基线 commit 300c3dc · 项目版本 V2.2.1 · 识别层/G1 已落地，下游闭环待确认*
