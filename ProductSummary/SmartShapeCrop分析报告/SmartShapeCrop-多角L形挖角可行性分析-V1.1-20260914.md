...
| 当前状态 | 已实施到第 1 步 | **识别层收敛解除 + G1 闸口 + 数据模型/几何 mask/手动多角 GUI 最小闭环已完成**；**自动识别多角回填（`all_corners` → 4 行 GUI 建议值）已完成**；**UI 输入后的四边实时边余量提示、G2 低置信度告警、G3 状态区摘要与预览图角位画框已完成**；素材边框补全二期仍未完成 | 工作区基于 `300c3dc` 的增量；专项 GUI 窄回归通过 |
...
- [ ] 素材边框补全 `core/lshape_border.py` 尚未逐凹角化，仍按旧单角 `cut_corner` 路径处理。
- [x] G2 低置信度告警已完成：状态区提示 + 识别 GroupBox 高亮，阈值为 0.75。
- [x] G3 第一阶段已完成：多角识别结果在状态区展示角位与建议尺寸。
- [x] G3 预览图画框与标签已完成；素材边框补全仍按旧单角 `cut_corner` 路径处理。
...
6. **下一步进入素材边框补全二期**：先只支持对角双挖角，逐角调用现有补边逻辑并增加重叠/冲突测试；确认稳定后再开放相邻角和四角同挖。
7. **随后补强真实草图验收**：覆盖低置信度、识别漏角、手动修改后标注同步、草图切换后 overlay 清除等场景。
8. 当前可交付边界：最多 4 角的**手动几何渲染 + 预览角位核对**可用；逐边退化守卫、实时余量反馈、G1/G2/G3 已完成；挖角边缘素材边框连续性仍是后续重点。

---

*SmartShapeCrop 多角 L 形挖角可行性分析 · 报告版本 V1.1 · 2026-09-14*
*当前基线 commit 300c3dc · 项目版本 V2.2.1 · 2026-09-15 工作区已完成手动多角几何闭环 + 自动识别回填 4 行 GUI + 实时边余量反馈 + G1/G2/G3 预览核对闭环；素材边框补全仍待实施*
