...
2. **已完成逐边几何约束**：`CropDesign.validate()` 现在按 `inner_rect` 的四条边检查相邻 cut 宽/高之和，统一保留 2cm 余量；下一步应把该错误反馈接到 GUI，避免用户只在生成时看到 `ValueError`。
3. **先补约束专项回归**：覆盖四条边、恰好达到余量、单角兼容、对角双挖角和四角组合；在此基础上再开放相邻角的 UI 入口。
4. **再补 G2/G3**：低自洽度告警；在预览或状态区展示每个识别角位与尺寸建议。不要把 `result.debug` 再丢弃。
5. **最后做素材边框补全二期**：先只支持对角双挖角，逐角调用现有补边逻辑并增加重叠/冲突测试；确认稳定后再开放相邻角和四角同挖。
6. 当前可交付边界：最多 4 角的**手动几何渲染**可用；逐边退化守卫已完成；对角双挖角为验收场景；自动识别多角回填已完成，挖角边缘素材边框连续性仍是后续二期重点。
