...
- [ ] G2/G3 置信度与强约束告警仍待补齐；当前已完成的是多角识别与 GUI 回填的最小闭环。
...
2. **下一步应先补逐边几何约束**：在 `CropDesign.validate()` 或统一几何校验入口中检查同一外边上相邻 cut 宽/高之和，阻止相邻 cut 相接、自交或退化。该项应先于开放相邻角。
3. **再补 G2/G3**：低自洽度告警；在预览或状态区展示每个识别角位与尺寸建议。不要把 `result.debug` 再丢弃。
4. **最后做素材边框补全二期**：先只支持对角双挖角，逐角调用现有补边逻辑并增加重叠/冲突测试；确认稳定后再开放相邻角和四角同挖。
5. 当前可交付边界：最多 4 角的**手动几何渲染**可用；对角双挖角为验收场景；自动识别多角回填已完成，挖角边缘素材边框连续性仍是后续二期重点。
