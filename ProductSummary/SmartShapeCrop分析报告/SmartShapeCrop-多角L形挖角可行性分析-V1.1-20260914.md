...
| 当前状态 | 已实施到第 1 步 | **识别层收敛解除 + G1 闸口 + 数据模型/几何 mask/手动多角 GUI 最小闭环已完成**；**自动识别多角回填（`all_corners` → 4 行 GUI 建议值）已完成**；**UI 输入后的四边实时边余量提示、G2 低置信度告警、G3 状态区角位核对摘要已完成**；预览图角位画框与素材边框补全二期仍未完成 | 工作区基于 `300c3dc` 的增量；专项窄回归通过 |
...
| **加装闸口** | 现有失效模式是「静默丢掉一个角，却报告识别成功」。已完成 G1、G2，以及 G3 的状态区核对摘要；预览图画框仍待补 | §6 |
...
| **G3<br>人工核对** | 识别结果的角位与建议尺寸 | 多角场景**恒开** | **已完成第一阶段**：状态区列出每个角位及建议宽×高；**未完成**：预览图画框并标角落标签 |
...
| 几何还原信息 | `result.debug` 已含 `geometry` / `concave_px` / `verts` / `bbox`（`:1278-1287`） | ⚠️ 多角候选已用于 G3 状态区摘要；预览图画框仍未接入 |
...
| G3 | `gui/lshape_panel.py`（`_apply_lshape_params`）状态栏摘要 | 已逐角列出「位置 + 建议尺寸」；预览图画框与标签仍待实现 |
...
- [ ] 完成 **G3 预览可视化**：利用 §7.4 的 `debug['verts']` / `concave_px` 做角位画框与标签
...
- [ ] G3 预览图画框与标签仍待补齐；素材边框补全仍按旧单角 `cut_corner` 路径处理。
...
5. **已完成：G2/G3 第一阶段**：低自洽度告警；状态区展示每个识别角位与建议尺寸。下一步仅补 G3 预览图画框与标签，并用真实草图回归标定 0.75 阈值。
6. **下一步优先做 G3 预览可视化**：先把 `result.debug['concave_px']` / `geometry` 转换为画布坐标，在预览图上画角位框和 `tl/tr/bl/br` 标签；该步完成后再进入素材边框补全，避免边框问题与识别核对问题同时排查。
7. **随后做素材边框补全二期**：先只支持对角双挖角，逐角调用现有补边逻辑并增加重叠/冲突测试；确认稳定后再开放相邻角和四角同挖。
8. 当前可交付边界：最多 4 角的**手动几何渲染**可用；逐边退化守卫、实时余量反馈、G1/G2 与 G3 状态摘要已完成；预览图角位可视化和挖角边缘素材边框连续性仍是后续重点。

---

*SmartShapeCrop 多角 L 形挖角可行性分析 · 报告版本 V1.1 · 2026-09-14*
*当前基线 commit 300c3dc · 项目版本 V2.2.1 · 2026-09-15 工作区已完成手动多角几何闭环 + 自动识别回填 4 行 GUI + 实时边余量反馈 + G2/G3 第一阶段告警摘要；G3 预览图可视化与素材边框补全仍待实施*
