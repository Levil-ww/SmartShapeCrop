...
### 阶段 6：多边 L 形完整化期（2026.09.14–09.15）

| 文件名 | 核心内容 | 关键产出 |
|---|---|---|
| `20260914-多角L形挖角（收敛点解除+G1闸口+切边补边越界修复）.md` | 解除识别层三个单角收敛点（`argmax`/`or 短路`/`max`）；G1 结构一致性闸口；切边补边越界修复（`inset_top`/`inset_right`） | 多角从可行性分析 → 落地 |
| `20260915-多角L形三期完整化与数据管线修复.md` | 数据模型多角化 → GUI 多行回填 → 四角同挖验收；OCR 逐角归属（`_resolve_cut_pair_per_corner`）；G1 常驻不变量；三轮草图数据识别 Bug 修复 | 多边 L 形三期满交付，501 测试 |

### 规划探讨（未实施）

| 文件名 | 内容 |
|---|---|
| `20260916-单边阶梯L形挖角可行性分析与实现建议.md` | **单边阶梯 L 形**（同一条外边上多级台阶）可行性分析（V2.0，含真实草图实测）。**核心发现：识别层数据基本全对**（`n_detected=2`、外框 93.5×55 完全正确、OCR 逐角读出 10/6.5/8.5/3.5 与草标注逐一吻合），**缺口仅在「凹角列表→形状」的表述方式**（两凹角都判 `tr`，无法区分级别）。⚠️ 当前**静默出错**：`success=True`、`g1_blocked=False`，但渲染形状与真实阶梯 **IoU 仅 0.9791**（第二级台阶丢失）。面板决策**集成不新开**；推荐 `CutRect(anchor, offset_x, offset_y, w, h)` 模型（旧模型为严格子集）；工期 **8–13 天**（原估 13–20 天，因识别层实测可用而下调） |
| `20260916-单边阶梯与多边L形在同一面板的区分设计.md` | 承接上篇，回答「都集成在 L 形面板，两种形状怎么区分」。**核心结论：不需要让用户选择形状 —— 形状是参数的必然结果。** 判据：**按锚定角（anchor）分组，每组各 1 个 → 多边 L 形；某组 ≥2 个 → 该角是阶梯；两者共存 → 混合（自动支持）**。该 `group_by_anchor()` 一个函数同时服务三件事：形状区分、**约束分层**（现有「同边尺寸求和」约束对阶梯是语义错误，须按组分层）、渲染遍历。UI：4 个平级行 → **父行（选角位）+ 可折叠子行（同角追加级）**，用户全程不被问「你要哪种形状」 |
