...
"""
packageV2.1.2.py
SmartShapeCrop V2.1.2 打包脚本（Python 版）

功能：
    将 main.py 打包为 Windows 桌面可执行文件（单文件模式，双击即可运行）：
      - exe 文件名：智能裁剪设计器V2.1.2
...
V2.1.2 相比 V2.1 的新增/优化功能：
      ...
      - 多洞草图解析：sketch_parser_multihole 独立模块 + 零侵入式扩展
      - 水池设计器 L 形挖角草图识别：lshape_sketch_parser 模块
...
使用方式：
    python packageV2.1.2.py                    # 单文件模式（默认，生成单个 exe，双击即可运行）
    python packageV2.1.2.py --onedir           # 目录模式（更稳定，若 onefile 有问题可用）
    python packageV2.1.2.py --debug            # 调试模式（显示控制台窗口，便于排查错误）
    python packageV2.1.2.py --clean            # 清理旧构建后打包
    python packageV2.1.2.py --no-tesseract     # 不内嵌 Tesseract（默认已内嵌；OCR 不可用时草图识别将失败，需自装 Tesseract）
...
def _build_cmd(mode: str, debug: bool, tesseract_src_dir: Path | None = None) -> list[str]:
    ...
    # hidden-import：显式声明隐藏导入（含 V2.1.2 全部新增模块）
...
def main() -> None:
    args = parse_args()
    print("=" * 60)
    print(f"  SmartShapeCrop V2.1.2 打包工具")
    print(f"  exe 名称: {APP_NAME}")
    print(f"  exe 图标: {ICON_FILE.name}")
