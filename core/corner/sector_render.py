...
def _redraw_border_on_corner(
    ...
    for d in range(total_border_depth):
        ...
        if not paint_inside_arc and src_arr is not None and d > 0:
            ...
            if len(d_local[0]) > 0:
                src_pixels_here = src_arr[d_local[0], d_local[1], :].astype(np.float64)
                color_diff = np.sqrt(np.sum((src_pixels_here - bg_f64) ** 2, axis=1))
                keep_mask = color_diff > 15.0  # 色差 > 15 才涂边框色
                if not np.any(keep_mask):
                    continue
                # 把 d_region 收紧到 keep_mask 子集
                full_mask = np.zeros_like(d_region, dtype=bool)
                full_mask[d_local[0][keep_mask], d_local[1][keep_mask]] = True
                d_region = full_mask
