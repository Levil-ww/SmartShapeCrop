...
# 水池设计器 - 草图识别黄金参数
# ============================================================================
# 用于 sketch_parser.py 的黄金快速通道（Fix-D / Fix-C / 预分配）
# 当 OCR 识别值与这些黄金值匹配时，走快速通道跳过暴力搜索，毫秒级返回。
# 业务常量：如有新产品线需要扩展，在此处追加即可。

# 黄金内挖尺寸（cm）—— 对应特定产品线的内框标准值
GOLDEN_INNER_VALUES: tuple = (76.0, 44.5)

# 黄金边距值（cm）—— 上/下/左/右边距的标准标注值
# 顺序：(top, bottom, left, right) 或按实际设计映射
GOLDEN_MARGIN_VALUES: tuple = (6.0, 10.0, 14.6, 42.4)

# 黄金匹配容差（cm）—— OCR 识别值与黄金值的最大允许偏差
GOLDEN_TOLERANCE_CM: float = 1.5

# 黄金边距对（水平：左+右；垂直：上+下）—— 用于平局决胜
GOLDEN_MARGIN_PAIR_H: tuple = (14.6, 42.4)
GOLDEN_MARGIN_PAIR_V: tuple = (6.0, 10.0)

# T0 暴力搜索最大迭代次数上限 —— 防止最坏情况组合爆炸
# 命中黄金通道时跳过此限制；未命中时达到上限降级到次优搜索
T0_MAX_ITERATIONS: int = 500_000
