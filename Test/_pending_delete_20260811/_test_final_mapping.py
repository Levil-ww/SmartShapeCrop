...
for d in range(min(R_total + 1, total_border_depth + 100)):
    ...
    for i in range(len(border_layers)):
        ...
        thickness = cum_next - cum_i
        
        if cum_i <= d < cum_next:
            if clipped >= thickness:
                next_cum = min(cum_next, total_border_depth)
                next_thickness = border_layers[i + 1][1] if i + 1 < len(border_layers) else 0
                src_depth = min(next_cum + next_thickness // 2, total_border_depth - 1)
            elif d < cum_i + clipped:
                src_depth = min(cum_i + thickness // 2, total_border_depth - 1)
            else:
                src_depth = min(d, total_border_depth - 1)
            break
