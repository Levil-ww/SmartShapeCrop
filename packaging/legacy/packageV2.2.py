...
"""
packageV2.2.py
SmartShapeCrop V2.2 打包脚本（Python 版）
...
V2.2 相比 V2.1.2 的新增/优化功能：
      - L 形挖角边框补全模块：core.lshape_border
        （池素材 L 形挖角后两条新边缘自动补全与素材一致的边框层，
          按外→内顺序依次涂色，支持多层嵌套与几何差集法绘制）
      - L 形挖角描边/色带/细边框结构重走线模块：core.lshape_border_route
        （针对蔓生花 / 中古雨林 / 庄园秘境等多层结构素材的 V13 路径扩展，
          1D 颜色剖面扫描自动分割有序层结构，三层封顶，内凹角 max(dx,dy) 分层）
      - L 形挖角独立 GUI 面板：gui.lshape_panel
        （把 L 形挖角相关 UI 与识别逻辑单开一个面板，
          新增草图上传 + 目标文件名输入 + 一键生成控件，
          通过信号委托给 PropertyPanel 的同名方法实现双向同步）
      - 修复 L 形挖角渲染三层问题：
          1) 外侧黑色边框未正确 L 形切割 → 通过填充 inner_rect 外区域
             使用 outer_bg_color 解决
          2) 挖角区域左下角残留米色色带 → 通过 inner_rect 内边 3%-12%
             深度环采样 bg_color 替换棕色解决
          3) 米色 L 形边框拐点不相交 → 跳过统一 10px 黑色边框绘制，
             将素材拉伸到 inner_rect 尺寸，使用 inner_rect/原始素材尺寸
             计算边框补全比例因子

V2.1.2 基础功能（继承）：
      - 水池设计器【多洞功能】：支持识别并解析草图中多个内挖孔洞
        （Phase A-E 五阶段算法消包络盒 + OCR 加权众数投票）
      - 多洞 GUI 参数面板：动态显示多个洞的尺寸与中距，支持手动修改
      - 多洞状态栏：显示各洞尺寸（洞1/洞2/...）与中间距
      - 多洞内孔边框：统一 10px 黑色边框线，采用几何差集法绘制
      - 圆角裁剪直角尾巴与白色空隙修复：统一 ring_region 与 d_region
        的 dist 上界为 r + 1.5，消除保护/绘制不一致区域
...
使用方式：
    python packageV2.2.py                    # 单文件模式（默认，生成单个 exe，双击即可运行）
    python packageV2.2.py --onedir           # 目录模式（更稳定，若 onefile 有问题可用）
    python packageV2.2.py --debug            # 调试模式（显示控制台窗口，便于排查错误）
    python packageV2.2.py --clean            # 清理旧构建后打包
    python packageV2.2.py --no-tesseract     # 不内嵌 Tesseract（默认已内嵌；OCR 不可用时草图识别将失败，需自装 Tesseract）
...
def _build_cmd(mode: str, debug: bool, tesseract_src_dir: Path | None = None) -> list[str]:
    ...
    # hidden-import：显式声明隐藏导入（含 V2.2 全部新增模块）
...
def main() -> None:
    args = parse_args()
    print("=" * 60)
    print(f"  SmartShapeCrop V2.2 打包工具")
    print(f"  exe 名称: {APP_NAME}")
    print(f"  exe 图标: {ICON_FILE.name}")
