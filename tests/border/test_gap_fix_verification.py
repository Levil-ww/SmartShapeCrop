...
def test_corner_smoothness_no_gap_fill():
    ...
    # 在左下角圆角区域（真实圆心 (r_px, h - r_px)，半径 ~177px）
    # 图像结构：外层黑边框(0-50px)、灰色间隙(50-70px)、内层红边框(70-100px)
    # [V2.2-fix 测试几何] 真圆心不是图角 (0, h)
...
def test_different_border_colors():
    ...
    for border_color, gap_color, desc in test_configs:
        ...
        # 检查是否有错误的颜色填充
        wrong_fill_count = 0
        # [V2.2-fix 测试几何] 真圆心是 (r_px, ±r_px)，不是距图角 40 像素
        _r_for_pos = int(2.0 / 2.54 * dpi)
        corner_positions = {
            'tl': (_r_for_pos, _r_for_pos),
            'tr': (w - _r_for_pos, _r_for_pos),
            'bl': (_r_for_pos, h - _r_for_pos),
            'br': (w - _r_for_pos, h - _r_for_pos),
        }
