...
- **GUI 不阻塞**：大图裁剪/导出运行于 QThread 后台线程；UI 参数修改采用防抖渲染（200ms 延迟、800ms 最大等待）；预览用 BILINEAR（快 3-5×），导出用 LANCZOS
...
│   │   └── sector_render.py        #     圆角弧线多层边框重绘（仅最外层 + 深色外层保护 + 伪影检测）
│   │
│   ├── parser/                     #   文件名解析子包
...
```bash
# 全部测试（实测 320 passed / 5 skipped，约 45 秒）
...
- **GUI 防抖渲染**：SpinBox 等参数修改 200ms 延迟 / 800ms 最大等待
...
所有业务常量集中在 `config.py` 单一来源：边框检测阈值、默认值（DPI=150、背景色白色、裁剪模式 simple_resize）、切割损耗（尺寸+1cm、圆角+0.5cm）、单位换算（cm↔px）、像素上限（2 亿，防御解压缩炸弹）、Tesseract 搜索路径模板与 **PathResolver** 跨平台路径解析（源码模式/打包模式差异统一处理）。
...
测试位于 `tests/` 目录，按模块分子目录组织，使用 pytest 框架。实测基线（2026-09-10）：**320 passed / 5 skipped**。
...
- **月度分类整理总结**（`ProductSummary/2026-08/`、`ProductSummary/2026-09/`）：
  - [20260817-任务分类整理总结.md](ProductSummary/2026-08/20260817-任务分类整理总结.md)
  - [20260818-19-任务分类整理总结.md](ProductSummary/2026-08/20260818-19-任务分类整理总结.md)
  - [20260820-21-任务分类整理总结.md](ProductSummary/2026-08/20260820-21-任务分类整理总结.md)
  - [20260826-28-任务分类整理总结.md](ProductSummary/2026-08/20260826-28-任务分类整理总结.md)
  - [20260829-任务分类整理总结.md](ProductSummary/2026-08/20260829-任务分类整理总结.md)
  - [20260831-0902-任务分类整理总结.md](ProductSummary/2026-08/20260831-0902-任务分类整理总结.md)
